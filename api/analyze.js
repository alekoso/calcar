...
- 2-4 пункти максимум. Краще 2 влучні, ніж 5 загальних.
...
{
 "vehicle": {"title":"Марка Модель Рік","year":2024,"fuel":"petrol|diesel|hybrid|electric","displacement_l":2.5,"engine":"2.5 л бензин, назва двигуна","transmission":"Автомат, 8 ст.","drive":"Повний AWD","trim":"осмислений рівень комплектації (M Sport, xLine, Premium) або null. НІКОЛИ не бери літери з назви моделі: 'i' з '540i' це НЕ trim","equipment":["опція 1","опція 2"],"premium_options":["опція 1"],"mileage_note":"пробіг з одометра якщо видно, милі та км"},
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lotPhotoUrls = (lot && Array.isArray(lot.images))
      ? lot.images.slice(0, 14).map(pickUrl).filter(u => typeof u === 'string' && /^https:/.test(u))
      : [];
    ...
    const buildContent = sources => {
      const c = [];
      for (const img of sources.slice(0, 14)) {
