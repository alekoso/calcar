...
Окремо склади "maintenance" — планове обслуговування за пробігом, за логікою досвідченого майстра: "машина вже розібрана — міняємо те, що скоро попросить заміни". Орієнтуйся на пробіг з одометра: масло+фільтри завжди; свічки, гальмівні колодки/диски, рідини, ремені/ролики, помпа — якщо пробіг близький до їх ресурсу; для гібрида — обслуговування системи охолодження інвертора. 3-6 позицій з цінами для України (запчастина+робота разом). Якщо пробіг невідомий — базовий набір (масло, фільтри, рідини).
  ...
  try {
    ...
    if (imgs.length === 0) {
      ...
      return res.status(200).json({
        ...
        mode: 'vin_only',
        _meta: { vin, analyzed_at: new Date().toISOString() }
      });
    }
