...
/* ---------- 4г2. Контекст рішення: детерміновані входи purchase_decision ----------
   ...
   змінюється. Рішення про покупку це синтез фактів ПРО АВТО: ризик +
   бажаність + пробіг + комплектація + обʼєктивний ціновий контекст. Код
   збирає факти детерміновано, модель ними МІРКУЄ. Жодного нового числового
   бала цей шар не вводить. Особистий контекст людини сюди не входить. */
...
/* ---------- межа обʼєктивності висновку ----------
   ЗВІТ = ПРО АВТО, ЧАТ = ПРО АВТО ДЛЯ ЦІЄЇ ЛЮДИНИ. Звіт (і його публічне
   посилання) можуть переслати іншій людині, тому висновок мусить бути
   однаковим для будь-кого, хто перевіряє те саме авто з тими самими
   доказами. Особистий контекст (бюджет, уподобання, сценарій використання,
   памʼять помічника, інші авто людини) живе лише в чаті: api/chat.js
   отримує його окремо, своїм запитом. У модель висновку він НЕ передається
...
export function applyDecisionLanguage(pd, { severity = null, lang = 'en' } = {}) {
  ...
  for (const k of ['questions_for_seller', 'missing_but_important']) {
...
export function sanitizeHistoricalVisual(hvRaw, photosSent) {
  ...
  return {
    ...
    /* потенційно структурна зона БЕЗ надійно видимого силового елемента:
       НЕ підтверджене структурне, у Score не входить, живе в risks/checklist */
...
- "questions_for_seller": список, відсортований за важливістю: найдорожче за ціною помилки перше. Аргументи "за", головні побоювання і обовʼязкові перевірки живуть у reasoning, risks і checklist, а не окремими списками.
...
- універсальні агрегатні списки в checklist: КОЖЕН пункт привʼязаний до знайденого факту або конкретної діри в даних ЦІЄЇ машини. Пункт, що пасує будь-якому авто цієї моделі, це брак;
...
СТРУКТУРА reasoning (2-4 абзаци, без заголовків і без нумерації, без роздування обсягу): сильна позиція одним рядком; чим саме цей екземпляр цікавий (головні плюси); що реально насторожує (головні мінуси); саме зважування цих факторів між собою; 1-3 конкретні речі, які мусять підтвердитись до купівлі; ціновий контекст із чесною атрибуцією (розрахунок CalCar за даними площадки); чіткий фінальний висновок. Текст має закривати роздуми покупця, а не перелічувати знахідки.
`;
const DECISION_PRINCIPLES = `
ПРИНЦИПИ РІШЕННЯ (коротко, замість прикладів):
1. Зважуй фактори за їх реальною вагою для покупця, а не перелічуй усе підряд: спершу те, що найдорожче за ціною помилки.
2. Один сильний плюс може компенсувати помірний ризик, але не підтверджений deal-breaker (доведене приховане ДТП з подушками, скрутка, структурне пошкодження, обман продавця).
3. Ціна впливає на рішення ЛИШЕ за наявності structured price_context; без нього про ринкову ціну не суди.
4. Не драматизуй невелике ДТП і не применшуй тяжке: формулювання йде за evidence, не за загальним враженням.
5. Обʼєктивний стан авто (Оцінка CalCar) і привабливість самої пропозиції це різні речі: не змішуй їх у тексті. Висновок не залежить від того, хто саме перевіряє авто.
6. Роби ясний висновок: чи варто далі розглядати саме цей екземпляр і які 1-3 умови реально змінюють рішення.
7. Не вигадуй фактів, яких нема в evidence: чого не перевірено, того не стверджуй ані як плюс, ані як мінус.
`;

/* блок КОНТЕКСТ РІШЕННЯ у промпті: те, що код зібрав детерміновано з
   обʼєктивних фактів про авто (значимість пробігу). Особистого контексту
   людини тут немає за побудовою: див. objectiveDecisionContext.
...
function renderDecisionContext(dc) {
  ...
  return 'КОНТЕКСТ РІШЕННЯ (зібраний кодом, детермінований; це ВХІД для purchase_decision, а не текст для копіювання у звіт):\n'
    + parts.join('\n')
    + '\nЧого в цьому блоці нема, того не вигадуй.\n\n';
}
...
const MAIN_HISTORICAL_RULES = auction => auction && auction.photos_sent ? (auction.hv_provided ? `
ІСТОРИЧНІ КАДРИ (auction_photo_N, зроблені ДО ремонту, коли авто продавали пошкодженим) і КАНОНІЧНИЙ historical_visual у блоці HISTORICAL_VISUAL_EVIDENCE.
Канонічний historical_visual уже визначений окремим читанням ТИХ САМИХ кадрів і містить зони пошкоджень, тяжкість і глибину, структурні спостереження та SRS. Ці спостереження НЕ переоцінюй, НЕ визначай заново і НЕ підсилюй понад канонічний обʼєкт: він є джерелом правди про історичне пошкодження для auction.summary, risks, score_facts і purchase_decision; сумнів трактуй так, як він записаний там (indeterminate = невідомо).
Кадри передані тобі ЛИШЕ для задач, яких нема в окремому читанні:
- порівняння зони удару "до" з нинішніми фото "після": збіг відтінку, зазори, посадка деталей;
- видимі ознаки якості відновлення;
- узгодженість слів продавця про обсяг пошкоджень і ремонту з канонічним розбором: занижує, чесний чи перебільшує;
- поточний зовнішній результат ремонту.
Писати "аукціонні фото недоступні" заборонено: вони є. У відповіді historical_visual завжди null: код підставить канонічний обʼєкт.
...
"score_facts": СЛУЖБОВА класифікація знахідок для коду, на текст звіту НЕ впливає. Числовий бал (Оцінку CalCar) рахує код зі score_facts: у відповіді бал не пиши. Жорсткі правила класифікації:
...
- ОФІЦІЙНИЙ ЗАПИС ПРО ДТП (позначка "Був у ДТП", "Зафіксовано ДТП" у держ/історичному блоці, history_facts.accident_recorded) це ФАКТ із доказом (source historical_listing чи registry), НЕ "відсутність даних". Але MAJOR_REPAIR_UNVERIFIED НЕ ставиться через саме підтверджене ДТП невідомої тяжкості: потрібен ПОЗИТИВНИЙ доказ суттєвості (спрацьовані подушки, структурні пошкодження, кілька зон, серйозний опис damage у записі). НЕДОСТУПНІСТЬ фото чи матеріалів доказом суттєвості НЕ є. Відоме ДТП без доказів суттєвості: знахідку в score_facts не створюй, на бал воно не впливає, АЛЕ purchase_decision мусить помітно його відпрацювати: у reasoning і summary_short із точним текстом запису, а конкретну перевірку постав у checklist.
...
POSSIBLE STRUCTURAL: якщо historical_visual.possible_structural_damage = true, цей факт НЕ губиться: додай його у risks (рівень за загальною сортовкою тяжкості, НЕ автоматично першим: підтверджені flood/скрутка/VIN/structural і сильніші фактори мають вищий пріоритет) і в checklist з формулюванням "пошкодження в потенційно структурній зоні; стан силової частини за доступними фото підтвердити не можна" та конкретною перевіркою силової частини/геометрії на підйомнику. НЕ стверджуй структурне пошкодження як факт.
...
${DECISION_RULES}${decisionStyle === 'a' ? DECISION_PRINCIPLES : ''}
${proseSchema
    ? 'Відповідай ЛИШЕ валідним JSON без markdown, точно за схемою (усі ключі обовʼязкові, enum-значення точні, невідоме = null де дозволено):\n' + schemaProse({ hvProvided: !!(auction && auction.hv_provided) }) + '\n'
    : 'СХЕМА ВІДПОВІДІ задана структурно (response_format json_schema, strict): усі ключі обовʼязкові, enum-значення точно за схемою, невідоме = null там, де схема це дозволяє; семантика кожного поля описана в самій схемі, змістовні правила вище мають пріоритет над стислістю.\n'}
...
async function runCheck(req, res, job) {
  ...
  try {
    ...
    try {
      ...
      /* лише обʼєктивні факти про авто: тіло запиту (памʼять, бюджет,
         інші авто людини) сюди не читається взагалі */
      decisionContext = objectiveDecisionContext({ mileage });
...
{ type: 'text', text: 'CURRENT_VISUAL_EVIDENCE: ФОТО З ОГОЛОШЕННЯ (стан зараз). Нумерація: photo_1..photo_' + photoUrls.length + ' у порядку подачі, на неї посилаються evidence ref. '
    ...
    let mainRetries = 0;
    let mainBody = modelBody(content, true, mainSystem, mainFormat);
    let data = await callModel(mainBody, Math.max(100000, Math.min(240000, 268000 - (Date.now() - tRun))));

    /* endpoint/модель не приймає strict json_schema: ОДИН повтор із
       json_object і текстовою схемою в правилах (той самий контракт полів) */
    if (data?.error && /json_schema|response_format|schema|strict|structured/i.test(String(data.error.message || ''))) {
      console.log('[check] json_schema rejected, fallback to json_object:', String(data.error.message || '').slice(0, 160));
      mainStructured = 'fallback_json_object';
      mainMsg = { system: MAIN_RULES(auction, decisionStyle, auctionSearch, { proseSchema: true }), user: mainMsg.user };
      mainSystem = mainMsg.system;
      mainFormat = { type: 'json_object' };
      content[0] = { type: 'text', text: mainMsg.user };
      mainRetries++; mainBody = modelBody(content, true, mainSystem, mainFormat);
    ...
    if (data?.error && /reasoning_effort|unknown|unsupported|unrecognized/i.test(String(data.error.message || ''))) {
      mainRetries++; mainBody = modelBody(content, false, mainSystem, mainFormat);
      data = await callModel(mainBody, Math.max(60000, 250000 - (Date.now() - t0)));
    }
    ...
    if (data?.error && /image|url|download|fetch/i.test(String(data.error.message || ''))) {
      ...
      mainRetries++; mainBody = modelBody(content.filter(x => x.type === 'text'), true, mainSystem, mainFormat);
      data = await callModel(mainBody, Math.max(60000, 250000 - (Date.now() - t0)));
    }
    ...
    mark('main_analysis', Date.now() - t0, 'executed', { retries: mainRetries, structured: mainStructured, ai: aiUsage(data, mainBody), payload: mainPayload });
    ...
    /* КАНОНІЧНИЙ БАЛ: модель числовий бал не генерує (його нема в схемі
       відповіді). verdict.score для UI, публічного звіту, карток і чату
       ставить код зі Score v3: score_breakdown.final; нижче порога даних
       або при падінні скорингу це null, а не вигадане число */
    parsed.verdict = (parsed.verdict && typeof parsed.verdict === 'object' && !Array.isArray(parsed.verdict)) ? parsed.verdict : {};
    parsed.verdict.score = (parsed.score_breakdown && parsed.score_breakdown.score_available !== false && typeof parsed.score_breakdown.final === 'number')
      ? parsed.score_breakdown.final : null;
    ...
    /* possible structural: якщо модель не поклала його в risks/checklist,
       ...
       Перевірка йде в canonical checklist (він показується у звіті) */
    try {
      if (parsed.historical_visual && parsed.historical_visual.possible_structural_damage === true) {
        ...
        if (PS) {
          ...
          if (!parsed.checklist.some(mentions)) parsed.checklist.push(PS.check);
        }
      }
