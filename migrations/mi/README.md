...
| `024_safeupdate_temp_clear` | сумісність із `safeupdate` у сесіях PostgREST: сім очищень ТИМЧАСОВИХ таблиць дістають явний `where true`. Тіла функцій в усьому іншому ті самі; семантика MI не змінюється |
| `025_decoder_model_year_plausibility` | рік із декодера VIN отримує силу `vin_decoder` лише після перевірки правдоподібності (цикл коду 10-ї позиції, частковий декод); неправдоподібний рік зберігається тим самим джерелом з довірою low і знятий з розвʼязання через `invalidated_at` |
