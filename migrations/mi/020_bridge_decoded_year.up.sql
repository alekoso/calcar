-- Міграція 20: міст не пише рік оголошення як модельний рік.
--
-- Phase 7.2 показала: `vehicles.year` у продакшні це рік із оголошення
-- auto.ria (рік першої реєстрації), а не модельний рік. Для
-- WBAJB9C50JB049616 там 2017, хоча машина MY2018. Хибний рік гірший за
-- невідомий: кандидатних VMY для нього немає, і знання про мотор і
-- коробку зникає цілком, а не стає умовним.
--
-- Модельний рік тепер береться лише з декоду: `vehicles.model_year`
-- (Check пише його з ModelYear NHTSA) або з самого `nhtsa->>'ModelYear'`.
-- Евристики за 10-м символом VIN тут НЕМАЄ: це окреме рішення.
--
-- Решта функції без змін. Текст узято з міграції 018 і пропатчено
-- у двох місцях.
