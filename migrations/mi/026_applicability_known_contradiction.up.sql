-- Міграція 26: інваріант застосовності. UNKNOWN означає, що вимір справді
-- невідомий; відомий вимір, що суперечить застосовності, дає NO_MATCH.
--
-- Дві доведені семантичні помилки (картка 3 MI Catalog, 2026-09-15):
--
-- 1. Відомий модельний рік поза роками версії. Частковий шлях
--    mi.compile_pack (міграції 022, 024) будував пакет версії, навіть коли
--    mi.partial_candidates не залишав жодного VMY: Tucson 2015 отримував
--    пакет Tucson TL 2.4 2018..2021, Model 3 2017 пакет 2018..2023, 530i
--    2016 пакет 2017..2020. Знання без предиката року (версія, покоління,
--    права бренду) оцінювались APPLICABLE, а знання про компоненти
--    CONDITIONAL, тобто відомий суперечливий рік поводився як невідомий.
--    Виправлення: якщо рік відомий, версія має VMY і жоден VMY версії не
--    має цього року, компіляція повертає fragment_available false з
--    причиною version_excluded_by_model_year. Невідомий рік, а також ринок
--    і дата виробництва поводяться як раніше.
--
-- 2. Підтверджена несумісна родина компонента. Якір на варіант
--    (mi.eval_claim, mi.eval_check), якір на родину
--    (mi.anchor_other_result) і явні предикати component_variant і
--    component_family (mi.eval_predicate) шукали у ідентичності лише
--    компонент ТІЄЇ САМОЇ родини; якщо його не було, результат був UNKNOWN,
--    навіть коли та сама роль (мотор) зайнята відомим компонентом іншої
--    родини: знання B48 на машині з N63, знання Theta II 2.4 на машині з
--    Nu 2.0. Витоку в пакети не було лише завдяки області фрагмента.
--    Виправлення: mi.component_role_conflict. Роль родини береться з
--    каталогу комплектації (version_fitment). Якщо кожна така роль у слоті
--    ідентичності зайнята розвʼязаним компонентом іншої родини, результат
--    mi.status_to_result(статус, роль, false): NO_MATCH для підтвердженого
--    або сильного заводського припущення, ASSUMED_NO_MATCH для слабкого.
--    Роль other не вирішує нічого (у ній різні родини стоять поруч),
--    статус conflicted чи unresolved лишає UNKNOWN, родина без ролі в
--    каталозі лишає UNKNOWN.
--
-- Тексти функцій узято дослівно з останніх міграцій (eval_predicate 012,
-- eval_claim, eval_check і anchor_other_result 021, compile_pack 024) і
-- доповнено лише позначеними місцями. Схема, gate і дані не змінюються.

-- ---------- 1. Несумісна родина у ролі ----------

create or replace function mi.component_role_conflict(
  p_family bigint, p_identity jsonb, p_slot text)
returns jsonb language plpgsql stable as $$
declare r text; occ jsonb; res text; worst text := 'NO_MATCH'; v_basis text := 'confirmed'; v_obs text;
  n_roles int := 0;
begin
  if p_family is null then return null; end if;

  -- Компонент цієї родини у слоті вже є: це не конфлікт ролі, це питання
  -- варіанта, яке викликач розвʼязує сам.
  if exists (select 1 from jsonb_array_elements(coalesce(p_identity->'components', '[]'::jsonb)) e
              where e->>'slot' = p_slot
                and (select v.family_id from mi.component_variant v
                      where v.subject_id = (e->>'variant')::bigint) = p_family) then
    return null;
  end if;

  for r in select distinct f.role_code from mi.version_fitment f
             join mi.component_variant v on v.subject_id = f.variant_id
            where v.family_id = p_family and f.role_code <> 'other'
            order by 1 loop
    n_roles := n_roles + 1;
    select e into occ from jsonb_array_elements(coalesce(p_identity->'components', '[]'::jsonb)) e
     where e->>'slot' = p_slot and e->>'role' = r
       and (select v.family_id from mi.component_variant v
             where v.subject_id = (e->>'variant')::bigint) is distinct from p_family
       and (select v.family_id from mi.component_variant v
             where v.subject_id = (e->>'variant')::bigint) is not null
     order by case e->>'status' when 'confirmed' then 0 when 'assumed_factory' then 1 else 2 end
     limit 1;
    if occ is null then return null; end if;
    res := mi.status_to_result(occ->>'status', r, false);
    if res not in ('NO_MATCH', 'ASSUMED_NO_MATCH') then return null; end if;
    if res = 'ASSUMED_NO_MATCH' then
      worst := 'ASSUMED_NO_MATCH';
      v_basis := mi.status_to_basis(occ->>'status', r);
    elsif v_basis = 'confirmed' then
      v_basis := mi.status_to_basis(occ->>'status', r);
    end if;
    v_obs := coalesce(v_obs, occ->>'variant');
  end loop;

  if n_roles = 0 then return null; end if;
  return jsonb_build_object('result', worst, 'basis', v_basis, 'observed', v_obs);
end $$;

-- ---------- 2. Рік, що суперечить версії ----------

create or replace function mi.version_year_exclusion(p_identity jsonb)
returns jsonb language sql stable as $$
  select jsonb_build_object(
           'fragment_available', false,
           'reason', 'version_excluded_by_model_year',
           'version_id', (p_identity->>'version')::bigint,
           'model_year', (p_identity->>'model_year')::numeric::int,
           'version_model_years', (select jsonb_agg(distinct y.model_year order by y.model_year)
                                     from mi.version_market_year y
                                    where y.version_id = (p_identity->>'version')::bigint))
   where p_identity->>'version' is not null
     and p_identity->>'model_year' is not null
     and exists (select 1 from mi.version_market_year y
                  where y.version_id = (p_identity->>'version')::bigint)
     and not exists (select 1 from mi.version_market_year y
                      where y.version_id = (p_identity->>'version')::bigint
                        and y.model_year = (p_identity->>'model_year')::numeric::int);
$$;

-- ---------- 3. Оцінювачі (джерело: 012, 021) ----------
...
begin
  ...
  if p_pred.dimension = 'component_variant' then
    ...
    -- Міграція 26: роль цієї родини зайнята відомим компонентом ІНШОЇ родини.
    -- Це доведена несумісність, а не невідомість.
    other := mi.component_role_conflict(fam, p_identity, want_slot);
    if other is not null then
      return jsonb_build_object('result', other->>'result', 'basis', other->>'basis',
  ...
  if p_pred.dimension = 'component_family' then
    ...
    select e into hit from jsonb_array_elements(coalesce(p_identity->'components','[]'::jsonb)) e
     ...
     limit 1;
    if hit is null then
      -- Міграція 26: відомий компонент іншої родини у тій самій ролі.
      other := mi.component_role_conflict(p_pred.ref_subject_id, p_identity, want_slot);
      if other is not null then
        return jsonb_build_object('result', other->>'result', 'basis', other->>'basis',
                                  'observed', other->>'observed');
      end if;
...
declare
  ...
  other_anchor jsonb; other_ev jsonb; conflict jsonb;
begin
  ...
  if anchor is not null then
    ...
    if role_row is null then
      -- Міграція 26: роль родини якоря зайнята відомим компонентом іншої
      -- родини. Це доведена несумісність: NO_MATCH (або ASSUMED_NO_MATCH
      -- для слабкого заводського припущення), а не UNKNOWN.
      conflict := mi.component_role_conflict(fam, p_identity, 'current');
      if conflict is not null then
        role_status := conflict->>'result';
        group_results := group_results || role_status;
        if basis = 'confirmed' then basis := conflict->>'basis'; end if;
        log := log || jsonb_build_object('group', 0, 'dimension', 'component_variant',
                 'slot', 'current', 'operator', 'eq', 'expected', anchor::text,
                 'observed', conflict->>'observed', 'result', role_status,
...
begin
  ...
  if anchor is not null then
    ...
    select e into role_row from jsonb_array_elements(coalesce(p_identity->'components','[]'::jsonb)) e
     ...
     limit 1;
    if role_row is null then
      -- Міграція 26: та сама доведена несумісність, що в mi.eval_claim.
      conflict := mi.component_role_conflict(fam, p_identity, 'current');
      if conflict is not null then
        group_results := group_results || (conflict->>'result');
        if basis = 'confirmed' then basis := conflict->>'basis'; end if;
        log := log || jsonb_build_object('group', 0, 'dimension', 'component_variant',
                 'slot', 'current', 'operator', 'eq', 'expected', anchor::text,
                 'observed', conflict->>'observed', 'result', conflict->>'result',
                 'basis', conflict->>'basis');
      else
      group_results := group_results || 'UNKNOWN'::text;
      if basis = 'confirmed' then basis := 'unresolved'; end if;
...
begin
  if p_anchor->>'kind' = 'component_family' then
    select e into hit from jsonb_array_elements(coalesce(p_identity->'components', '[]'::jsonb)) e
     ...
     limit 1;
    if hit is null then
      -- Міграція 26: роль родини зайнята відомим компонентом іншої родини.
      -- Контекст родини описує заводську архітектуру, тому для нього
      -- вирішує заводський слот, для решти знання поточний.
      hit := mi.component_role_conflict(sid, p_identity,
               case when p_propagation = 'family_context' then 'factory' else 'current' end);
      if hit is not null then
        return jsonb_build_object('group', 0, 'dimension', 'component_family',
          'slot', case when p_propagation = 'family_context' then 'factory' else 'current' end,
          'operator', 'eq', 'expected', sid::text, 'observed', hit->>'observed',
          'result', hit->>'result', 'basis', hit->>'basis');
      end if;
