-- CalCar Model Intelligence, міграція 11: buyer-метадані у staging.
--
-- Phase 3.1 показала втрату даних між дослідженням і канонічним клеймом:
...
-- еталонних атомів не потрапляв нікуди.
--
-- Міграція аддитивна: нових таблиць і типів немає, окрема buyer-сутність
-- НЕ вводиться, семантична модель не переглядається.
--
-- Розподіл обовʼязків такий самий, як у решті схеми:
--   домен значення   -> CHECK на таблиці (1..5);
--   судження про
--   якість знання    -> правило у mi.check_gate.
-- Тому вимога «contested без пояснення не публікується» живе у gate, а не
-- у CHECK: дослідження має право покласти у staging чернетку, яку
-- перевірка якості потім відхилить. На канонічному рівні це вже накрито
-- claim_contested_ck з міграції 05.

-- ---------- 1. Колонки staging ----------

alter table mi.candidate_claim
  add column if not exists proposed_buyer_importance   smallint,
  add column if not exists proposed_buyer_implication_en text,
  add column if not exists proposed_contested          boolean not null default false,
  add column if not exists proposed_contested_note_en  text;

comment on column mi.candidate_claim.proposed_buyer_importance is
  'Наскільки це знання здатне змінити рішення покупця, 1..5. Переноситься у mi.claim.buyer_importance без замовчування.';
comment on column mi.candidate_claim.proposed_buyer_implication_en is
  'Що це означає для покупця. Переноситься у mi.claim.buyer_implication_en дослівно.';

alter table mi.candidate_claim drop constraint if exists candidate_claim_importance_ck;
alter table mi.candidate_claim add constraint candidate_claim_importance_ck
  check (proposed_buyer_importance is null or proposed_buyer_importance between 1 and 5);

-- ---------- 2. Перевірка якості ----------

-- Змінені лише два місця: додано правило про обовʼязкову важливість і
-- правило про пояснення суперечливості. Жодне наявне правило не змінене
-- і не послаблене.
...
begin
  ...
  -- 10. Важливість для покупця задана явно.
  -- Канонічний клейм не має для неї замовчування, і мовчазна підстановка
  -- середнього значення знищила б сенс, закладений дослідженням.
  ok := c.proposed_buyer_importance is not null;
  rules := rules || jsonb_build_object('code', 'buyer_importance_set', 'ok', ok,
    'detail', 'buyer importance must be stated explicitly, there is no default');
  passed := passed and ok;

  -- 11. Суперечливе знання пояснює, у чому саме суперечка.
  ok := coalesce(c.proposed_contested, false) = false
        or coalesce(btrim(c.proposed_contested_note_en), '') <> '';
  rules := rules || jsonb_build_object('code', 'contested_note', 'ok', ok,
    'detail', 'a contested claim must say what is disputed');
  passed := passed and ok;

  -- 12. Правила за типом знання
...
-- Єдина змінена частина: buyer-поля беруться з кандидата, а не з
-- константи. Константу buyer_importance = 3 прибрано; замовчування немає
-- ніде, бо правило gate не пускає кандидата без явної важливості.
...
begin
  ...
  -- Навіть під override канонічний клейм не може лишитись без важливості:
  -- колонка обовʼязкова, і мовчазної підстановки тут немає свідомо.
  if c.proposed_buyer_importance is null then
    raise exception 'candidate % has no buyer importance and there is no default', p_candidate_id;
  end if;
  ...
  insert into mi.claim (subject_id, knowledge_type, text_en, normalized_assertion,
      ...
      layer, causal_status, contested, contested_note_en,
      propagation, propagation_note, status, dedup_key,
  ...
  values (c.resolved_subject_id, c.proposed_knowledge_type, c.text_en,
      ...
      coalesce(c.proposed_confidence, 'low'),
      c.proposed_buyer_implication_en, c.proposed_buyer_importance,
      c.proposed_layer, c.proposed_causal_status,
      coalesce(c.proposed_contested, false), c.proposed_contested_note_en,
      c.proposed_propagation,
