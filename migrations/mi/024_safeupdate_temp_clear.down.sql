-- Відкат міграції 24: повернути тіла функцій до версії без `where true`.
--
-- УВАГА: після відкату виклик через PostgREST знову падатиме з
-- SQLSTATE 21000 `DELETE requires a WHERE clause`, бо роль
-- `authenticator` вантажить `safeupdate`. Відкат має сенс лише як
-- частина повного відкату Phase 7.
--
-- Тіла беруться дослівно з міграцій 013, 015 і 022.
