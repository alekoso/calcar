...
(async () => {
     ...
     верифікатор комплектації вимкнений, безпечний fallback */
  {
    ...
    const iFeedWait = chk.indexOf('const r = await Promise.race([cvShadow'), iPrompt = chk.indexOf('let mainMsg = PROMPT(');
  ...
  if (vFrames.map(f => f.gallery_index).join() !== '5' || vFrames.some(f => !f.high)) errs.push('кадри для перевірки одометра: ' + JSON.stringify(vFrames.map(f => f.gallery_index)));
  const vFrames2 = CV.odometerVerifyFrames({ dashboard: { odometer_reading: { gallery_index: 5 } } }, CV.normalizeFrames([{ gallery_index: 5, url: a }, { gallery_index: 8, url: d }]), { 5: 'dashboard', 8: 'dashboard' });
  if (vFrames2.map(f => f.gallery_index).join() !== '5,8') errs.push('другий кадр приладової панелі має додаватись');
  if (!/ПОВЕРНИ його завжди/.test(CV.ODOMETER_VERIFIER_RULES)) errs.push('верифікатор мовчить замість читання з нижчою впевненістю');
