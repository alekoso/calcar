...
create or replace function pg_temp.adopt(p_key text, p_subject_id bigint)
returns bigint language plpgsql as $$
declare v_id bigint;
begin
  ...
  insert into key_map (key, subject_id) values (p_key, p_subject_id);
  return p_subject_id;
end $$;

create or replace function pg_temp.adopt_src(p_key text, p_source_id bigint)
returns bigint language plpgsql as $$
declare v_id bigint;
begin
  ...
  insert into source_map (key, source_id) values (p_key, p_source_id);
  return p_source_id;
end $$;
