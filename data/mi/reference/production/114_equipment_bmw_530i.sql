...
-- Джерело: 114_equipment_bmw_530i.sql
--
...
select mi_load.adopt('brand:bmw', (select subject_id from mi.brand where name = 'BMW'));
select mi_load.adopt('vmy:530ix_us_2018', (
...
select mi_load.adopt_src('S-G-OFF-02', (
...
insert into mi.equipment_item (subject_id, brand_id, equipment_key, oem_code, item_kind, name_en, value_tier) values
  (mi_load.mk('equip:bmw_zda', 'equipment_item', 'BMW Driving Assistance Package'),
   mi_load.sid('brand:bmw'), 'driving_assistance_package', 'ZDA', 'package', 'Driving Assistance Package', 'notable'),
  (mi_load.mk('equip:bmw_zdb', 'equipment_item', 'BMW Driving Assistance Plus Package'),
   mi_load.sid('brand:bmw'), 'driving_assistance_plus_package', 'ZDB', 'package', 'Driving Assistance Plus Package', 'high_value'),
  (mi_load.mk('equip:bmw_zdh', 'equipment_item', 'BMW Dynamic Handling Package'),
   mi_load.sid('brand:bmw'), 'dynamic_handling_package', 'ZDH', 'package', 'Dynamic Handling Package', 'high_value'),
  (mi_load.mk('equip:bmw_zls', 'equipment_item', 'BMW Luxury Seating Package'),
   mi_load.sid('brand:bmw'), 'luxury_seating_package', 'ZLS', 'package', 'Luxury Seating Package', 'high_value'),
  (mi_load.mk('equip:bmw_zpx', 'equipment_item', 'BMW Executive Package'),
   mi_load.sid('brand:bmw'), 'executive_package', 'ZPX', 'package', 'Executive Package', 'high_value'),
  (mi_load.mk('equip:bmw_zpk', 'equipment_item', 'BMW Parking Assistance Package'),
   mi_load.sid('brand:bmw'), 'parking_assistance_package', 'ZPK', 'package', 'Parking Assistance Package', 'notable'),
  (mi_load.mk('equip:bmw_zlp', 'equipment_item', 'BMW Lighting Package'),
   mi_load.sid('brand:bmw'), 'lighting_package', 'ZLP', 'package', 'Lighting Package', 'notable'),
  (mi_load.mk('equip:bmw_zmp', 'equipment_item', 'BMW M Sport Package'),
   mi_load.sid('brand:bmw'), 'm_sport_package', 'ZMP', 'package', 'M Sport Package', 'notable'),
  (mi_load.mk('equip:bmw_610', 'equipment_item', 'BMW Head-up Display'),
   mi_load.sid('brand:bmw'), 'head_up_display', '610', 'option', 'Head-up Display', 'high_value'),
  (mi_load.mk('equip:bmw_5at', 'equipment_item', 'BMW Active Driving Assistant Plus'),
   mi_load.sid('brand:bmw'), 'active_driving_assistant_plus', '5AT', 'option',
   'Active Driving Assistant Plus with Active Cruise Control with Stop & Go', 'high_value'),
  (mi_load.mk('equip:bmw_688', 'equipment_item', 'BMW Harman Kardon surround sound system'),
   mi_load.sid('brand:bmw'), 'harman_kardon_surround_sound', '688', 'option', 'Harman Kardon surround sound system', 'notable'),
  (mi_load.mk('equip:bmw_6f1', 'equipment_item', 'BMW Bowers & Wilkins Diamond Surround Sound System'),
   mi_load.sid('brand:bmw'), 'bowers_wilkins_diamond_surround_sound', '6F1', 'option',
   'Bowers & Wilkins Diamond Surround Sound System', 'high_value'),
  (mi_load.mk('equip:bmw_6uk', 'equipment_item', 'BMW Night Vision with Pedestrian Detection'),
   mi_load.sid('brand:bmw'), 'night_vision_pedestrian_detection', '6UK', 'option',
   'Night Vision with Pedestrian Detection', 'high_value'),
  (mi_load.mk('equip:bmw_2vh', 'equipment_item', 'BMW Integral Active Steering'),
   mi_load.sid('brand:bmw'), 'integral_active_steering', '2VH', 'option', 'Integral Active Steering', 'high_value'),
  (mi_load.mk('equip:bmw_223', 'equipment_item', 'BMW Dynamic Damper Control'),
   mi_load.sid('brand:bmw'), 'dynamic_damper_control', '223', 'option', 'Dynamic Damper Control', 'notable'),
  (mi_load.mk('equip:bmw_453', 'equipment_item', 'BMW Front ventilated seats'),
   mi_load.sid('brand:bmw'), 'front_ventilated_seats', '453', 'option', 'Front ventilated seats', 'high_value'),
  (mi_load.mk('equip:bmw_4t7', 'equipment_item', 'BMW Front massaging seats'),
   mi_load.sid('brand:bmw'), 'front_massaging_seats', '4T7', 'option', 'Front massaging seats', 'high_value'),
  (mi_load.mk('equip:bmw_zx3', 'equipment_item', 'BMW Surround View with 3D View'),
   mi_load.sid('brand:bmw'), 'surround_view_3d', 'ZX3', 'option', 'Surround View with 3D View', 'high_value'),
  (mi_load.mk('equip:bmw_552', 'equipment_item', 'BMW Icon Adaptive Full LED Headlights'),
   mi_load.sid('brand:bmw'), 'icon_adaptive_led_headlights', '552', 'option', 'Icon Adaptive Full LED Headlights', 'notable'),
  (mi_load.mk('equip:bmw_6wb', 'equipment_item', 'BMW Dynamic Digital Instrument Cluster'),
   mi_load.sid('brand:bmw'), 'dynamic_digital_instrument_cluster', '6WB', 'option', 'Dynamic Digital Instrument Cluster', 'notable'),
  (mi_load.mk('equip:bmw_6fh', 'equipment_item', 'BMW Rear-seat entertainment Professional'),
   mi_load.sid('brand:bmw'), 'rear_seat_entertainment_professional', '6FH', 'option',
...
select mi_load.sid('vmy:530ix_us_2018'), mi_load.sid(v.item), v.availability::mi.availability,
       v.price, case when v.price is null then null else 'USD' end,
       case when v.requires is null then null else mi_load.sid(v.requires) end,
       mi_load.src('S-G-OFF-02')
  from (values
...
update mi.equipment_availability a
  ...
  from (select mi_load.sid(item) as item_id, array_agg(mi_load.sid(pkg) order by pkg) as pkgs
          from (values
         ...
         group by item) v
 where a.vmy_id = mi_load.sid('vmy:530ix_us_2018') and a.item_id = v.item_id;
...
select mi_load.sid(v.item), v.alias, lower(v.alias), v.lang, v.kind::mi.alias_kind, 'brand',
       mi_load.sid('brand:bmw'), case when v.kind = 'official' then mi_load.src('S-G-OFF-02') end
  from (values
 ...
 where not exists (select 1 from mi.subject_alias s
                    where s.scope_key = mi_load.sid('brand:bmw') and s.alias_norm = lower(v.alias));

-- ---------- Візуальні ознаки ----------

insert into mi.equipment_visual_hint (equipment_id, cue_key, specificity, description_en, negative_note_en)
select mi_load.sid(v.item), v.cue, v.spec::mi.visual_specificity, v.descr, v.neg
  from (values
