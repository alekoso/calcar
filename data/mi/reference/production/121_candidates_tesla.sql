...
-- Джерело: 121_candidates_tesla.sql
--
...
select mi_load.stage('T-001', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-01'), mi_load.ev('S-T-SPEC-01'))));

select mi_load.stage('T-002#a', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-01'), mi_load.ev('S-T-LEG-01'))));

select mi_load.stage('T-002#b', 'tesla', 'ver:p85d', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-LEG-01'), mi_load.ev('S-T-SPEC-01'))));

select mi_load.stage('T-003#a', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OWN-01', 'thread:tmc-50187'))));

select mi_load.stage('T-003#b', 'tesla', 'ent:ludicrous', 'owner_pattern', 'high',
...
select mi_load.stage('T-004', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-02'))));

select mi_load.stage('T-005', 'tesla', 'var:modem_3g', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      mi_load.p_date('production_date', null, date '2015-05-31')),
    'note', 'Without the upgrade the phone application does not reach the car away from a wireless network.',
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-03'))));

select mi_load.stage('T-006', 'tesla', 'vmy:p85d_us_2015', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-04'), mi_load.ev('S-T-OWN-02'))));

select mi_load.stage('T-007', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-05'))));

select mi_load.stage('T-008', 'tesla', 'ver:p85d', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-06'))));

-- ---------- B2. Приводи ----------

select mi_load.stage('T-010', 'tesla', 'issue:ldu_coolant_leak', 'known_issue', 'high',
  ...
  jsonb_build_object(
    'applic', jsonb_build_array(
      mi_load.p_attr('has_rotor_coolant_seal', 'true')),
    'note', 'A dry sensor today does not guarantee the seal tomorrow.',
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-03', 'thread:tmc-327284', '{"mileage_km":120000}'::jsonb),
      mi_load.ev('S-T-OWN-04', 'thread:tmc-316883', '{"mileage_km":135000}'::jsonb),
      mi_load.ev('S-T-SPEC-02'), mi_load.ev('S-T-SPEC-03'))));

select mi_load.stage('T-011#a', 'tesla', 'issue:ldu_coolant_leak', 'specialist_practice', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-SPEC-03'),
      mi_load.ev('S-T-OWN-04', 'thread:tmc-316883', '{"mileage_km":135000}'::jsonb))));

select mi_load.stage('T-011#b', 'tesla', 'issue:ldu_coolant_leak', 'owner_pattern', 'medium',
  'Owners treat the cost of a rear drive unit replacement as a known and budgeted figure rather than as an unexpected event.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-04', 'thread:tmc-316883', '{"mileage_km":135000}'::jsonb))));

select mi_load.stage('T-012', 'tesla', 'issue:ldu_coolant_leak', 'owner_pattern', 'medium',
...
select mi_load.stage('T-013', 'tesla', 'issue:ldu_milling', 'owner_pattern', 'medium',
  'A bearing milling noise from the rear drive unit led to warranty replacements between 55,000 and 90,000 miles.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-05', 'thread:tmc-158252', '{"mileage_km":88000}'::jsonb),
      mi_load.ev('S-T-OWN-06', 'thread:tmc-322489', '{"mileage_km":145000}'::jsonb))));

select mi_load.stage('T-014', 'tesla', 'issue:sdu_gear_whine', 'owner_pattern', 'medium',
  'The front drive unit whines at the reduction gear with isolated replacements, and there is no pattern of leaks from it.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-07', 'thread:tmc-sdu-whine', '{"mileage_km":100000}'::jsonb))));

select mi_load.stage('T-015', 'tesla', 'issue:half_shaft', 'owner_pattern', 'medium',
...
select mi_load.stage('T-020#a', 'tesla', 'var:pack_85_gen1', 'official_fact', 'high',
...
select mi_load.stage('T-020#b', 'tesla', 'var:pack_85_gen1', 'specialist_practice', 'high',
...
select mi_load.stage('T-021#a', 'tesla', 'var:pack_85_gen1', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-08', 'thread:tmc-285721', '{"age_years":9,"mileage_km":164000}'::jsonb))));

select mi_load.stage('T-021#b', 'tesla', 'var:pack_85_gen1', 'specialist_practice', 'medium',
  ...
  jsonb_build_object(
    'causal', 'observed_association',
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-05'))));

select mi_load.stage('T-022#a', 'tesla', 'issue:batterygate', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      mi_load.p_ref('generation', 'gen:ms_pre_facelift')),
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-09', 'thread:tmc-154976', '{"age_years":7}'::jsonb),
      mi_load.ev('S-T-LEG-02'))));

select mi_load.stage('T-022#b', 'tesla', 'issue:batterygate', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-LEG-02'), mi_load.ev('S-T-OWN-09'))));

select mi_load.stage('T-023', 'tesla', 'issue:chargegate', 'known_issue', 'medium',
...
select mi_load.stage('T-024', 'tesla', 'issue:bms_u029', 'known_issue', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-11', 'thread:tmc-300568', '{"mileage_km":63000,"age_years":8}'::jsonb),
      mi_load.ev('S-T-SPEC-06'))));

select mi_load.stage('T-025', 'tesla', 'issue:pack_condensation', 'known_issue', 'medium',
  ...
  jsonb_build_object(
    'applic', jsonb_build_array(
      mi_load.p_tag('condition_tag', 'humid_climate')),
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-12', 'thread:tmc-313395', '{"mileage_km":108000}'::jsonb))));

select mi_load.stage('T-026#a', 'tesla', 'issue:isolation_fault', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-13', 'thread:tmc-292083', '{"age_years":8}'::jsonb))));

select mi_load.stage('T-026#b', 'tesla', 'issue:isolation_fault', 'specialist_practice', 'medium',
...
select mi_load.stage('T-027', 'tesla', 'issue:contactor_weld', 'specialist_practice', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-07'))));

select mi_load.stage('T-028#a', 'tesla', 'var:pack_85_gen1', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-14', 'thread:tmc-300568'))));

select mi_load.stage('T-028#b', 'tesla', 'var:pack_85_gen1', 'owner_pattern', 'medium',
...
select mi_load.stage('T-029', 'tesla', 'maint:pack_charging_habit', 'owner_practice', 'medium',
...
select mi_load.stage('T-030#a', 'tesla', 'chk:pack_can_health', 'specialist_practice', 'high',
  ...
  jsonb_build_object(
    'causal', 'supported_cause',
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-08'))));

select mi_load.stage('T-030#b', 'tesla', 'chk:pack_can_health', 'owner_practice', 'high',
  ...
  jsonb_build_object(
    'causal', 'observed_association',
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-15', 'thread:tmc-battery-health'))));

select mi_load.stage('T-031#a', 'tesla', 'maint:pack_coolant', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-07'))));

select mi_load.stage('T-031#b', 'tesla', 'maint:ms_brake_fluid', 'official_fact', 'high',
...
select mi_load.stage('T-031#c', 'tesla', 'maint:ms_desiccant', 'official_fact', 'high',
...
select mi_load.stage('T-040#a', 'tesla', 'gen:ms_pre_facelift', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-08'), mi_load.ev('S-T-SPEC-09'))));

select mi_load.stage('T-040#b', 'tesla', 'state:ccs_retrofit', 'specialist_practice', 'high',
  ...
  jsonb_build_object(
    'causal', 'supported_cause',
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-09'))));

select mi_load.stage('T-041#a', 'tesla', 'gen:ms_pre_facelift', 'specialist_practice', 'medium',
  ...
  jsonb_build_object(
    'causal', 'supported_cause',
    'applic', jsonb_build_array(
      mi_load.p_tag('market_operated', 'EU', 1),
      mi_load.p_tag('market_operated', 'UA', 1)),
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-09'))));

select mi_load.stage('T-041#b', 'tesla', 'gen:ms_pre_facelift', 'owner_pattern', 'medium',
  'Owners who moved a North American car to Europe report that charging becomes a project of its own.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-16', 'thread:tmc-ccs2-europe', '{"age_years":8}'::jsonb))));

select mi_load.stage('T-042#a', 'tesla', 'ent:sc01', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-17', 'thread:tmc-sc-transfer'),
      mi_load.ev('S-T-PRESS-01'))));

select mi_load.stage('T-042#b', 'tesla', 'ent:sc01', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-17', 'thread:tmc-sc-transfer', '{"age_years":8}'::jsonb))));

select mi_load.stage('T-043', 'tesla', 'issue:entitlement_removal', 'known_issue', 'high',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(mi_load.ev('S-T-PRESS-02'))));

select mi_load.stage('T-044', 'tesla', 'gen:ms_pre_facelift', 'calcar_synthesis', 'medium',
  ...
  jsonb_build_object(
    'applic', jsonb_build_array(mi_load.p_tag('market_operated', 'UA')),
    'ev', jsonb_build_array(mi_load.ev('S-T-PRESS-03'))));

-- ---------- B5. Медіаблок, екрани, асистенти ----------

select mi_load.stage('T-050#a', 'tesla', 'issue:mcu1_emmc', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-09'), mi_load.ev('S-T-SPEC-10'))));

select mi_load.stage('T-050#b', 'tesla', 'issue:mcu1_emmc', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-09'))));

select mi_load.stage('T-051', 'tesla', 'issue:cluster_flicker', 'owner_pattern', 'medium',
...
select mi_load.stage('T-052', 'tesla', 'issue:yellow_border', 'known_issue', 'high',
...
select mi_load.stage('T-053#a', 'tesla', 'state:mcu2_retrofit', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-11'))));

select mi_load.stage('T-053#b', 'tesla', 'state:mcu2_retrofit', 'owner_pattern', 'high',
  'Owners who paid for the newer media unit describe the retrofit as worth the money.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-18', 'thread:tmc-mcu2-ap1', '{"age_years":7}'::jsonb))));

select mi_load.stage('T-054', 'tesla', 'var:ap1', 'official_fact', 'high',
...
select mi_load.stage('T-060', 'tesla', 'issue:air_strut_leak', 'known_issue', 'medium',
  'The front air struts leak, the compressor is overworked and fails, and the way to see it is an overnight settling test.',
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-SPEC-12'),
      mi_load.ev('S-T-OWN-05', 'thread:tmc-158252', '{"mileage_km":88000}'::jsonb))));

select mi_load.stage('T-061', 'tesla', 'issue:fore_aft_link', 'known_issue', 'high',
  ...
  jsonb_build_object(
    'applic', jsonb_build_array(
      mi_load.p_date('production_date', date '2013-09-01', date '2017-08-31')),
    'ev', jsonb_build_array(mi_load.ev('S-T-PRESS-04'))));

select mi_load.stage('T-062#a', 'tesla', 'maint:ms_brake_cleaning', 'official_fact', 'high',
  'The manufacturer prescribes annual cleaning and lubrication of the caliper pad slides and the parking brake calipers.',
  jsonb_build_object(
    'layer', 'official',
    'value_kind', 'interval',
    'value', '{"years":1}'::jsonb,
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-10'))));

select mi_load.stage('T-062#b', 'tesla', 'issue:parking_brake_seize', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    'applic', jsonb_build_array(mi_load.p_tag('condition_tag', 'salt_climate')),
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-19', 'thread:tmc-parking-brake', '{"age_years":8}'::jsonb))));

select mi_load.stage('T-063', 'tesla', 'issue:camber_wear_21', 'owner_pattern', 'medium',
...
select mi_load.stage('T-064', 'tesla', 'issue:door_handle_gen2', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-20', 'thread:tmc-handle-harness', '{"age_years":8}'::jsonb),
      mi_load.ev('S-T-MKT-01'))));

select mi_load.stage('T-065', 'tesla', 'issue:ptc_heater_gen1', 'known_issue', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-21', 'thread:tmc-353740', '{"age_years":6,"mileage_km":56000}'::jsonb),
      mi_load.ev('S-T-SPEC-13'))));

select mi_load.stage('T-066', 'tesla', 'issue:dcdc_12v', 'known_issue', 'medium',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(mi_load.ev('S-T-SPEC-14'))));

select mi_load.stage('T-067', 'tesla', 'issue:pano_roof_leak', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-22', 'thread:tmc-162011', '{"age_years":9}'::jsonb),
      mi_load.ev('S-T-OWN-22', 'thread:tmc-131989', '{"age_years":10}'::jsonb),
      mi_load.ev('S-T-OWN-22', 'thread:tmc-95201'))));

-- ---------- B7. Володіння і порівняння ----------

select mi_load.stage('T-070', 'tesla', 'ver:p85d', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-23', 'thread:tmc-efficiency-2015'),
      mi_load.ev('S-T-OWN-23', 'thread:tmc-winter-consumption'),
      mi_load.ev('S-T-OWN-23', 'thread:tmc-winter-range'))));

select mi_load.stage('T-071', 'tesla', 'ver:p85d', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-24', 'thread:tmc-158252', '{"mileage_km":88000}'::jsonb),
      mi_load.ev('S-T-OWN-24', 'thread:tmc-278390', '{"mileage_km":135000}'::jsonb))));

select mi_load.stage('T-072', 'tesla', 'ver:p85d', 'owner_pattern', 'medium',
...
select mi_load.stage('T-073#a', 'tesla', 'var:pack_90_gen1', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    'ev', jsonb_build_array(
      mi_load.ev('S-T-OWN-25', 'thread:tmc-90-degradation'),
      mi_load.ev('S-T-OWN-25', 'thread:tmc-90-alike'),
      mi_load.ev('S-T-OWN-25', 'thread:tmc-range-compared'))));

select mi_load.stage('T-073#b', 'tesla', 'var:pack_90_gen1', 'specialist_practice', 'medium',
...
select mi_load.stage('T-074', 'tesla', 'line:model_s', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(mi_load.ev('S-T-OFF-11'))));
