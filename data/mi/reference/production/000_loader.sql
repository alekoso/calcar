...
create schema if not exists mi_load;
...
-- mi_load.key_map тримає відповідність ключа і subject_id.

create table if not exists mi_load.key_map (
  key        text primary key,
  subject_id bigint not null
);

create table if not exists mi_load.source_map (
  key       text primary key,
...
create table if not exists mi_load.atom_map (
...
-- зникнення атома бути не може: він або в mi_load.atom_map, або тут.
create table if not exists mi_load.atom_note (
...
create or replace function mi_load.mk(p_key text, p_kind mi.subject_kind, p_label text)
...
begin
  ...
  insert into mi_load.key_map (key, subject_id) values (p_key, v_id);
  return v_id;
end $$;

create or replace function mi_load.sid(p_key text)
...
create or replace function mi_load.src(p_key text)
returns bigint language plpgsql stable as $$
declare v_id bigint;
begin
...
create or replace function mi_load.mksrc(
...
begin
  ...
  insert into mi_load.source_map (key, source_id) values (p_key, v_id);
  return v_id;
end $$;
...
create or replace function mi_load.stage(
...
declare
  ...
  e jsonb;
begin
  if exists (select 1 from mi_load.atom_map where task_ref = p_task_ref) then
  ...
  insert into mi_load.atom_map (task_ref, atom_id, card, candidate_id)
  values (p_task_ref, split_part(p_task_ref, '#', 1), p_card, v_id);
  return v_id;
end $$;

-- Атом, який свідомо не стає кандидатом.
create or replace function mi_load.note_atom(
  p_task_ref text, p_card text, p_status text, p_reason text)
returns void language plpgsql as $$
begin
  insert into mi_load.atom_note (task_ref, atom_id, card, status, reason)
  values (p_task_ref, split_part(p_task_ref, '#', 1), p_card, p_status, p_reason);
end $$;

-- ---------- Читабельні предикати ----------

create or replace function mi_load.p_ref(p_dim text, p_key text, p_group int default 1,
...
returns jsonb language sql immutable as $$
  select jsonb_build_object('dimension', p_dim, 'operator', 'eq',
    'ref_subject_id', mi_load.sid(p_key), 'group_no', p_group,
    'config_scope', p_scope, 'include_revisions', p_include_rev);
$$;

create or replace function mi_load.p_tag(p_dim text, p_tag text, p_group int default 1,
...
create or replace function mi_load.p_attr(p_key text, p_value text, p_group int default 1)
returns jsonb language sql immutable as $$
...
create or replace function mi_load.p_num(p_dim text, p_from numeric, p_to numeric,
...
create or replace function mi_load.p_date(p_dim text, p_from date, p_to date,
                                          p_group int default 1)
returns jsonb language sql immutable as $$
  select jsonb_build_object('dimension', p_dim, 'operator', 'in_range',
...
-- Доказ.
create or replace function mi_load.ev(p_src text, p_group text default null,
