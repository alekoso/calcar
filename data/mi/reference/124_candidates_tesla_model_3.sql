...
select pg_temp.stage('M-001', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      pg_temp.p_num('model_year', 2018, 2019, 1),
      pg_temp.p_tag('market_sold', 'US', 2)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-01', null, null, 'supports', 'Model 3 Long Range AWD 2018: range 310, 120/112/116 MPGe, 28.9462 kWh/100 mi, 147 and 188 kW AC 3-Phase, All-Wheel Drive.'),
      pg_temp.ev('S-M-OFF-02', null, null, 'supports', 'Model 3 Long Range AWD 2019: the same rating, vehicle 41190.'))));

select pg_temp.stage('M-002', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2020, 2020, 1), pg_temp.p_tag('market_sold', 'US', 2)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-03', null, null, 'supports', 'Model 3 Long Range AWD 2020: range 322, 124/116/121 MPGe, 27.96 kWh/100 mi.'))));

select pg_temp.stage('M-003', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2021, 2021, 1), pg_temp.p_tag('market_sold', 'US', 2)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-04', null, null, 'supports', 'Model 3 Long Range AWD 2021: range 353, 141/127/134 MPGe, 25.1 kWh/100 mi, 98 and 195 kW AC 3-Phase, charge240 11.2 kW.'),
      pg_temp.ev('S-M-OFF-03', null, null, 'context', 'The 2020 entry lists 10.0 kW for the standard charger.'))));

select pg_temp.stage('M-004', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2022, 2023, 1), pg_temp.p_tag('market_sold', 'US', 2)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-05', null, null, 'supports', 'Model 3 Long Range AWD 2022: range 358, 134/126/131 MPGe, charge240 11.5 kW.'),
      pg_temp.ev('S-M-OFF-06', null, null, 'supports', 'Model 3 Long Range AWD 2023: the same rating, vehicle 46204.'))));

select pg_temp.stage('M-005', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-28', null, null, 'supports', 'OtherEngineInfo: Dual Motor - Standard; Series and Trim empty; BatteryKWh 79.50 for the 2022 pattern.'))));

select pg_temp.stage('M-006', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-07', 'doc:nvlw-2026', null, 'supports', 'Model 3 and Model Y Long Range or Performance - 8 years or 120,000 miles (192,000 km), whichever comes first, with minimum 70% retention of Battery capacity over the warranty period.'))));

select pg_temp.stage('M-007', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'high',
...
select pg_temp.stage('M-008', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-15', null, null, 'supports', 'Production Dates: JUL 15, 2017 - SEP 30, 2020. The recall population includes all Model Year 2017 to 2020 Model 3 vehicles.'),
      pg_temp.ev('S-M-REV-03', null, null, 'context', 'Photographs of a refreshed Model 3 in Fremont with a production date of October 8, 2020.'))));

select pg_temp.stage('M-009', 'm3_lr_awd', 'fam:m3_thermal', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-08', null, null, 'supports', 'Vehicles manufactured between approximately July 2017 and October 2020 do not have a heat pump ... Vehicles manufactured afterward have a heat pump.'))));

select pg_temp.stage('M-010', 'm3_lr_awd', 'fam:m3_lv', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-08', null, null, 'supports', 'Vehicles manufactured in Gigafactory Shanghai before approximately October 2021, and in the Fremont Factory before approximately December 2021, are equipped with a Lead-Acid low voltage battery. Vehicles manufactured after these dates are equipped with a Lithium-Ion low voltage battery. The low voltage lead-acid battery for North American vehicles is AtlasBX / Hankook 85B24LS 12V 45Ah. It is your responsibility to monitor the low voltage battery health. Damage to the low voltage battery due to running out of range is not covered by the warranty.'))));

select pg_temp.stage('M-011', 'm3_lr_awd', 'var:m3_pack_2021', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-28', null, null, 'supports', 'BatteryKWh 79.50 for the model year 2022 pattern 5YJ3E1EB1NF.'),
      pg_temp.ev('S-M-REV-02', null, null, 'context', 'Registration information shared by buyers confirms that the new battery packs have a capacity of 82 kWh in the Model 3 Long Range AWD; roughly a 5% increase in energy density enabled by the new cells.'))));
...
-- лишається у staging.
select pg_temp.stage('M-012', 'm3_lr_awd', 'fam:tesla_ap', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-AGG-02', null, null, 'supports', 'In August 2017, Tesla announced that HW2.5 included a secondary processor node ... The first availability of HW3 was April 2019. The HW3 system board is the same physical size as the HW2.5 board.'),
      pg_temp.ev('S-M-REV-06', null, null, 'supports', 'HW3 was introduced in April 2019, while HW2.5 appeared on Teslas built between August 2017 and March 2019.'))));

select pg_temp.stage('M-013', 'm3_lr_awd', 'var:ap25', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-24', null, null, 'supports', 'Tesla is recalling certain 2018-2019 Model S, Model X, and 2017-2020 Model 3 vehicles equipped with Autopilot Computer 2.5 and operating certain firmware releases. The rearview image may not immediately display when the vehicle begins to reverse. Tesla will perform an over-the-air software update.'))));

-- Перехід медіаблока з Intel на AMD відомий лише з оглядів: чесний блок.
select pg_temp.stage('M-014', 'm3_lr_awd', 'fam:tesla_mcu', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-REV-04', null, null, 'supports', 'The first Model 3 and Y vehicles with MCU3 (Infotainment Computer with AMD Ryzen) are now being produced and delivered in North America, December 28, 2021.'),
      pg_temp.ev('S-M-REV-05', null, null, 'supports', 'MCU 2 was offered in 2017 (Model 3) through 2022; Tesla has not offered an upgrade path or retrofit to transition from MCU 2 to MCU 3.'))));

-- ---------- M2. Права ----------

select pg_temp.stage('M-015', 'm3_lr_awd', 'ent:m3_autopilot', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-29', null, null, 'supports', 'All Tesla vehicles now come with Autopilot bundled as a standard feature; Model 3 Standard Plus previously cost 37,500 dollars plus 3,000 dollars for the Autopilot option.'),
      pg_temp.ev('S-M-OWN-15', 'thread:tmc-295372', null, 'context', 'Autopilot being standard with all Teslas did not start until after April 2019; if you see a price for an upgrade for 3k, you do not have autopilot at all.'))));

select pg_temp.stage('M-016', 'm3_lr_awd', 'issue:m3_no_basic_ap', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-15', 'thread:tmc-295372', '{"age_years":4}'::jsonb, 'supports', 'I just purchased my first 2019 Tesla Model 3 Standard Range ... I do see an upgrade option that has Autosteer included but it is 3000.00.'),
      pg_temp.ev('S-M-OWN-14', 'thread:tmc-349214', '{"age_years":2}'::jsonb, 'supports', 'A 2023 Model 3 did not have basic Autopilot, which required 1000 dollars as an upgrade. The only hint is if you see Autopilot Safety Features included, this is confirmation that in fact Autopilot is NOT included.'))));

select pg_temp.stage('M-017', 'm3_lr_awd', 'ent:fsd', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-30', null, null, 'supports', 'When you transfer Enhanced Autopilot or Full Self-Driving (Supervised) from your current vehicle to your new vehicle, your current vehicle will only have access to basic Autopilot features.'),
      pg_temp.ev('S-M-OWN-18', 'thread:tmc-344013', null, 'supports', 'The transfer is only to a new vehicle from Tesla. You cannot transfer FSD from a lease vehicle.'))));

select pg_temp.stage('M-018', 'm3_lr_awd', 'issue:m3_fsd_gone', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-20', 'thread:tmc-210298', '{"age_years":1}'::jsonb, 'supports', 'He checked with his management team and was advised that they will not transfer the acceleration boost. He also advised if you sell the car to Tesla they will remove the acceleration boost before reselling it ... FSD purchase is the same policy, and always has been. The only way to sell with that feature is privately.'),
      pg_temp.ev('S-M-OWN-18', 'thread:tmc-344013', '{"age_years":2}'::jsonb, 'supports', 'The transfer is only to a new vehicle from Tesla. So, no transfer to a used vehicle purchased from Tesla.'))));

-- Acceleration Boost відомий лише з огляду: чесний блок.
select pg_temp.stage('M-019', 'm3_lr_awd', 'ent:accel_boost', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-REV-11', null, null, 'supports', 'Tesla offered Long Range dual-motor Model 3 owners who had software version 2019.40.2 the option to purchase a 2,000 dollar Acceleration Boost software upgrade, advertised to reduce 0-60 mph time from 4.4 s to 3.9 s.'))));

select pg_temp.stage('M-020', 'm3_lr_awd', 'ent:m3_pc_life', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-16', 'thread:tmc-357443', '{"age_years":8}'::jsonb, 'supports', 'They grandfathered all cars ordered up to June 30, 2018 with free premium connectivity for the life of the vehicle. It transfers to subsequent owners when the car is sold private party; the app now says it is not transferable to another owner.'),
      pg_temp.ev('S-M-OWN-17', 'thread:tmc-220846', '{"age_years":3}'::jsonb, 'supports', 'Right after my used 2018 M3 LR RWD was delivered on 1/2/21 ... I was informed I had a 1-month free trial of premium connectivity. A Tesla support rep told me that this did not apply to resales after a certain date.'))));

select pg_temp.stage('M-021', 'm3_lr_awd', 'issue:m3_salvage_fast_charging', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_ref('model_line', 'line:model_3', 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-27', 'thread:tmc-217848', '{"age_years":2}'::jsonb, 'supports', 'Anyone with a salvage title model 3 should expect this to happen at some point ... Supercharging is permanently disabled. A Salvage-Titled Vehicle HV Safety Inspection may be performed at the customer expense.'),
      pg_temp.ev('S-M-OWN-29', 'thread:tmc-338751', '{"age_years":5}'::jsonb, 'supports', 'I have a 2019 salvage title Model 3. I have had it for 2.5 years. I lost SC capability about 2 years ago, so I have been using the CCS1 adapter to DC fast charge.'),
      pg_temp.ev('S-M-OWN-28', 'thread:tmc-189858', null, 'supports', 'Salvage titles get fast charging disabled, per Teslas unsupported vehicle policy.'),
      pg_temp.ev('S-M-REV-07', null, null, 'context', 'Supercharging and/or fast charging through 3rd party chargers of the Salvage-Titled vehicle is permanently disabled, memo of February 2020.'),
      pg_temp.ev('S-M-REV-08', null, null, 'context', 'The salvaged vehicles would need to pass two inspections: the Salvage-Titled Vehicle High Voltage Safety Inspection and the Salvage-Titled Vehicle Fast-Charging Safety Inspection. Fast charging will not be enabled if the customer declines to authorize the required repairs.'),
      pg_temp.ev('S-M-REV-10', null, null, 'context', 'Several salvaged Model 3 owners chimed in to say they also gained access to the Supercharger network, August 2021.'))));

-- ---------- M3. Autopilot і софт покоління ----------

select pg_temp.stage('M-022', 'm3_lr_awd', 'gen:m3', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-32', null, null, 'supports', 'Tesla Vision eliminates the forward-facing radar starting in May 2021 from the Autopilot hardware package on Model 3 and Model Y vehicles built for the North American market.'),
      pg_temp.ev('S-M-AGG-01', null, null, 'context', 'The adaptive cruise control radar sensor was eliminated in April 2021 and the ultrasonic sensors used for park assist were eliminated in October 2022 and replaced with Tesla Vision.'))));

select pg_temp.stage('M-023', 'm3_lr_awd', 'issue:m3_phantom_braking', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2021, 2022, 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-25', null, null, 'supports', 'The Office of Defects Investigation has received 354 complaints alleging unexpected brake activation in 2021-2022 Tesla Model 3 and Model Y vehicles ... the vehicle unexpectedly applies its brakes while driving at highway speeds.'),
      pg_temp.ev('S-M-OFF-26', null, null, 'supports', 'This Preliminary Evaluation is closed without a manufacturer action. Date Closed: 06/29/2026.'))));

-- ---------- M4. Відклики з фізичним ремонтом ----------

select pg_temp.stage('M-024', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
...
select pg_temp.stage('M-025', 'm3_lr_awd', 'issue:m3_trunk_harness', 'known_issue', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-15', null, null, 'context', 'Tesla Service will inspect the trunk harness on affected vehicles for wear.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11676225', '{"age_years":7}'::jsonb, 'supports', 'Backup camera recall was not fixed properly or the new parts have same issue. As a result the backup camera was not functioning.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11690203', '{"age_years":7}'::jsonb, 'supports', 'I believe my car should be included in the recall for trunk harness recall. My alert and issue is the rear camera being off due to rear trunk ruining the wiring harness.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11619569', '{"age_years":6}'::jsonb, 'supports', 'The rear camera routine fails to activate. I wonder if this related to the trunk harness issue. I tried contact Tesla, but they want to charge for a repair.'))));

select pg_temp.stage('M-026', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      pg_temp.p_num('model_year', 2018, 2021, 1),
      pg_temp.p_date('production_date', date '2018-01-05', date '2021-04-20', 2)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-16', null, null, 'supports', 'Number of potentially involved: 2,791. Estimated percentage with defect: 2%. Production Dates: JAN 03, 2019 - APR 20, 2021. The front suspension lateral link is attached to the sub-frame using two fasteners. If a fastener is not secured to the correct specification, the fastener may loosen over time or separate from the sub-frame.'),
      pg_temp.ev('S-M-OFF-17', null, null, 'supports', 'Number of potentially involved: 422. Production Dates: JAN 05, 2018 - MAR 30, 2019. From service data covering January 2018 through March 2023, Tesla identified 25 warranty claims.'))));

select pg_temp.stage('M-027', 'm3_lr_awd', 'issue:m3_lateral_link', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2018, 2020, 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-27', null, null, 'supports', 'The Office of Defects Investigation has received 156 complaints alleging a suspension failure in model year 2018-2020 Model 3 and MY 2021-2023 Model Y vehicles. The suspension failure involves the detachment of the front lower lateral link ... The subject failures for this investigation exceed the scope of these recalls and do not appear to be related to the production issue that prompted those recalls.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11754256', '{"mileage_km":163700}'::jsonb, 'supports', 'I am the owner of a 2018 Tesla Model 3. At approximately 101,728 miles, my vehicle experienced a severe front suspension failure, in direct relation to NHTSA Preliminary Evaluation PE26006.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11757214', '{"age_years":8}'::jsonb, 'supports', 'I am reporting a front suspension failure on my 2018 Tesla Model 3 Dual Motor ... While driving slowly in a parking lot, I heard a clunk and felt the front wheel.'))));

select pg_temp.stage('M-028', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      pg_temp.p_num('model_year', 2019, 2021, 1),
      pg_temp.p_date('production_date', date '2018-12-16', date '2021-03-16', 2)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-18', null, null, 'supports', 'Number of potentially involved: 5,974. Select MY 2019-21 Model 3 vehicles with similar manufacturing records, DEC 16, 2018 - MAR 16, 2021. The brake calipers are attached to the wheel knuckle using two fasteners ... the caliper makes contact with the inner surface of the wheel rim.'))));

select pg_temp.stage('M-029', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      ...
      pg_temp.p_date('production_date', date '2018-07-06', date '2020-03-21', 2)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-19', null, null, 'supports', 'Number of potentially involved: 5,530. Select MY 2018-20 Model 3 vehicles with similar manufacturing records, JUL 06, 2018 - MAR 21, 2020. During assembly, one or both fasteners may not have been properly secured.'))));

select pg_temp.stage('M-030', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-20', null, null, 'supports', 'Both components may have been incorrectly reassembled after disassembly during a service action. 105 occurrences of incorrect reassembly in the United States were confirmed to have occurred between May 24, 2019 and August 15, 2022. Number of potentially involved: 24,064.'))));

select pg_temp.stage('M-031', 'm3_lr_awd', 'var:m3_heatpump', 'official_fact', 'high',
...
select pg_temp.stage('M-032', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(
      pg_temp.p_num('model_year', 2023, 2023, 1),
      pg_temp.p_date('production_date', date '2023-02-28', date '2023-10-11', 2)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-22', null, null, 'supports', 'On certain vehicles equipped with electronic power-assisted steering and operating with a software release prior to 2023.38.4, an overvoltage breakdown may overstress motor drive components on the printed circuit board. Production Dates: FEB 28, 2023 - OCT 11, 2023. Number of potentially involved: 376,241.'),
      pg_temp.ev('S-M-OWN-36', 'odi:11554611', '{"mileage_km":3700}'::jsonb, 'context', 'Vehicle started displaying warning messages: steering assist reduced and lane departure avoidance features unavailable, at 2,300 miles.'))));

select pg_temp.stage('M-033', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2023, 2023, 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-23', null, null, 'supports', 'NHTSA acknowledgement of recall 23V-434, pyrotechnic battery disconnect, 2023 Model 3 and Model Y.'),
      pg_temp.ev('S-M-OFF-24', null, null, 'supports', 'Tesla is recalling certain 2023 Model 3 and Model Y vehicles. The pyrotechnic battery disconnect may be defective. Tesla Service will replace the pyrotechnic battery disconnect, free of charge.'))));

select pg_temp.stage('M-034', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-24', null, null, 'supports', 'Tesla will perform an over-the-air software update, free of charge (22V037, 22V045, 22V063, 22V235, 22V702, 22V844, 23V085, 23V838, 24V051, 24V153, 24V376, 24V554, 24V935, 26V283). 26V507000: The headlight low beams may be too bright and exceed the maximum light output. The remedy is currently under development. Owner notification letters are expected to be mailed October 3, 2026.'))));

-- ---------- M5. Тепло ----------

select pg_temp.stage('M-035', 'm3_lr_awd', 'issue:m3_heatpump_sensor', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-11', null, null, 'supports', 'Some Model 3 and Model Y vehicles may be equipped with refrigerant pressure/temperature (P/T) sensors in the heat pump that can fault over time. If symptoms are present, replace all 3 refrigerant P/T sensors.'),
      pg_temp.ev('S-M-OFF-12', null, null, 'supports', 'Campaign Bulletin SB-21-18-002 R6, August 17, 2022, 2020-2021 Model 3, Model Y heat pump: Replace all 3 refrigerant P/T sensors. Correction time 1.00.'),
      pg_temp.ev('S-M-OWN-01', 'thread:tmc-217576', '{"mileage_km":2000}'::jsonb, 'supports', 'My 2021 LR AWD with 2000 km failed yesterday in -5 degree weather here in Ontario. Tesla service diagnosed problem as hardware related.'),
      pg_temp.ev('S-M-OWN-34', 'odi:11509108', '{"mileage_km":29500}'::jsonb, 'supports', 'AC/Heat pump total failure at 18,317 miles. Due to cold weather in Michigan and simply running the car, the heat pump went out entirely ... the windows could not be defrosted.'),
      pg_temp.ev('S-M-REV-13', null, null, 'context', 'In March of this year Tesla issued an internal service bulletin SB-21-18-002 to replace refrigerant pressure/temperature sensors in the heat pumps of 2020 and 2021 Model 3 and Model Y cars.'))));

select pg_temp.stage('M-036', 'm3_lr_awd', 'issue:m3_heatpump_sensor', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-34', 'odi:11711146', '{"mileage_km":135200}'::jsonb, 'supports', 'HVAC system went out at 7500ish miles. It was repaired under warranty, and a fix was administered through software by tesla. My HVAC has gone out again at 84k miles for the same reason.'),
      pg_temp.ev('S-M-OWN-34', 'odi:11499129', '{"mileage_km":90100}'::jsonb, 'supports', 'The supermanifold failed, causing the front defroster to not work properly, resulting in loss of visibility in freezing temperatures. This is a common problem with 2021/2022 model 3/model y with the new heat pump.'),
      pg_temp.ev('S-M-OWN-02', 'thread:too-20148', '{"age_years":1}'::jsonb, 'supports', 'When the compressor cuts off because of low suction pressure it probably does not come back on, because the pressure sensor is a safety device; you get an error in the cabin that the HVAC system needs service.'))));

select pg_temp.stage('M-037', 'm3_lr_awd', 'issue:m3_ptc_failure', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-24', 'thread:tmc-255178', '{"mileage_km":86900}'::jsonb, 'supports', 'Eventually I had to pay 850 dollars for a new PTC heater as my car is out of warranty. 54k, my car spent about 52k miles in northern Michigan. Another poster: Tesla service is quoting 1200 dollars for PTC replacement, out of warranty, 175k miles.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11625537', '{"mileage_km":102400}'::jsonb, 'supports', 'PTC Heater failed at 63,617 miles after 6 years of use. Warning message: Vehicle may not restart. Repair quoted 1,104.18 dollars.'),
      pg_temp.ev('S-M-OWN-26', 'thread:too-29888', '{"mileage_km":81847}'::jsonb, 'supports', 'On February 14th 2023, 3 years and 2 months into ownership, the PTC heater and Superbottle failed. Mileage was 81,847 km. I paid the 2,158.09 dollars to have it repaired.'),
      pg_temp.ev('S-M-OWN-25', 'thread:tmc-325149', '{"age_years":4}'::jsonb, 'context', 'My heater stopped blowing warm air two weeks ago on my 2020 model 3+; told the cost of PTC heater replacement is about 800 euro but an OTA update 2024.8.9 has solved the issue.'))));

-- ---------- M6. 12 В батарея ----------

select pg_temp.stage('M-038', 'm3_lr_awd', 'issue:m3_lv_lead_acid', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-03', 'thread:tmc-205672', '{"age_years":2}'::jsonb, 'supports', 'I am in the really hot Palm Desert area. I just had both M3 require batteries at the same time. Common denominator: 2 years old. 12v warning occurs and the following morning the vehicle will not start.'),
      pg_temp.ev('S-M-OWN-04', 'thread:tmc-248855', '{"age_years":2}'::jsonb, 'supports', 'I also got the message. Right around the 24 month mark. You run the real risk of walking out there one morning and the car not starting, and needing to be towed to the service center, lots of reports of that with model 3 12V battery failure.'),
      pg_temp.ev('S-M-OWN-05', 'thread:tmc-302725', '{"age_years":2}'::jsonb, 'supports', 'My 2021 Tesla model 3 12 volt battery just died without any warning. Only two years old as well as my neighbours 2021 Y. Lead-acid batteries are generally good for four to five years; a very warm climate can significantly shorten that.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11503755', '{"age_years":5}'::jsonb, 'supports', 'I received many system malfunction errors on the screen, stating such that the 12v battery needed replacing, while the car had been charging overnight.'))));

-- ---------- M7. Підвіска і кузов ----------

select pg_temp.stage('M-039', 'm3_lr_awd', 'issue:m3_uca_creak', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2018, 2020, 1)),
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-13', null, null, 'supports', 'On certain Model 3, Model Y, and Model X vehicles, there may be a water ingress path to both front upper control arm ball joints that, over time, could possibly lead to surface corrosion ... a creaking sound coming from the front suspension when steering at low speeds and under high loads. This is exclusively an NVH condition only and does not result in premature failure of the ball joints. Reseal the area around the ball joints using urethane.'),
      pg_temp.ev('S-M-OWN-09', 'thread:too-18675', '{"age_years":1}'::jsonb, 'supports', 'There is some sort of creaking/ticking noise coming from the front right side of my 2021 model 3 long-range. It only happens at low speeds.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11756085', '{"age_years":8}'::jsonb, 'supports', 'Lots of creaking and grinding noises coming from both front upper control arm bushings.'),
      pg_temp.ev('S-M-REV-12', null, null, 'context', 'Tesla has begun contacting some Model 3 owners in an effort to proactively address a common upper control arm issue; the fix involves resealing the upper control arms with a urethane paste.'))));

select pg_temp.stage('M-040', 'm3_lr_awd', 'issue:m3_trunk_leak', 'known_issue', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-10', null, null, 'supports', 'On some Model 3 vehicles, water may have leaked inside the trunk area through the trunk lid. Inspect inside the trunk area for signs of water leaks that came through the trunk lid. If this symptom is present, reseal the trunk lid. Model Year 2017 - 2023, Model 3, Build Location Fremont.'),
      pg_temp.ev('S-M-OWN-10', 'thread:tmc-337277', '{"age_years":4}'::jsonb, 'supports', 'So I have a 2020 model 3 long range and when it rains water will drip into the trunk around the seal which has been replaced multiple times by Tesla and yearly when it rains it still leaks. The car is now out of warranty.'),
      pg_temp.ev('S-M-OWN-11', 'thread:too-31421', '{"age_years":1}'::jsonb, 'supports', 'I have a 2022 Model 3 and I am experiencing a trunk leak. After it rains and I open the trunk lid, I notice water leaking in, dripping, into the trunk on the passenger side. I have taken the car to Tesla twice already.'),
      pg_temp.ev('S-M-OWN-33', 'odi:11574947', '{"age_years":4}'::jsonb, 'supports', 'The back trunk seal fails at the rear of the vehicle allowing water to spill into the trunk and deep trunk and cause flooding. I have had it repaired multiple times and the technician says that is a known issue.'))));

select pg_temp.stage('M-041', 'm3_lr_awd', 'issue:m3_charge_port_door', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-23', 'thread:too-30216', '{"age_years":5}'::jsonb, 'supports', 'The magnet corner piece fell off and the door does not open or close. Very common issue for the 2018 Model 3. Being out of warranty, you are going to have to pay to have your charge port mechanism replaced. The cost was around 300-350 dollars.'),
      pg_temp.ev('S-M-OWN-21', 'thread:tmc-171685', '{"age_years":2}'::jsonb, 'supports', 'Starting this evening, my charging door will not open on my Model 3 ... service center replaced the door.'),
      pg_temp.ev('S-M-OWN-22', 'thread:tmc-355606', '{"age_years":8}'::jsonb, 'supports', 'I have a 2018 Model 3 LR and the charge port door is giving me a lot of issue. I replaced the charge port with one from Tesla and the issue is still the same ... they came back with a 1,500 dollar estimate to replace the front body controller.'))));

-- ---------- M8. Пак і зарядка ----------

select pg_temp.stage('M-042', 'm3_lr_awd', 'issue:m3_bms_a079', 'owner_pattern', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-34', 'odi:11736600', '{"mileage_km":222100}'::jsonb, 'supports', '2021 M3 138,000 miles. Alerted error code BMS_a074 and BMS_a079. After reading forums, this seems to be a known issue with 2021 in particular.'),
      pg_temp.ev('S-M-OWN-34', 'odi:11711847', '{"mileage_km":92100}'::jsonb, 'supports', 'The high-voltage traction battery pack has failed twice on my 2021 Tesla Model 3, both times while the vehicle was under the original battery warranty: at approximately 15,100 miles and at approximately 57,222 miles.'),
      pg_temp.ev('S-M-OWN-34', 'odi:11657017', '{"mileage_km":43500}'::jsonb, 'supports', 'My 21 M3P 27K miles threw BMS_a079. I have never used a supercharger and rarely charge over 80%. Vehicle will no longer charge.'),
      pg_temp.ev('S-M-OWN-34', 'odi:11741063', '{"age_years":5}'::jsonb, 'supports', 'BMS_a079 error. Battery is no longer able to charge. High voltage battery will need to be replaced.'),
      pg_temp.ev('S-M-OWN-12', 'thread:tmc-356233', null, 'supports', 'Getting these errors. Cell 28 showing under 4 V. Can anyone replace it or replace the entire HV battery.'),
      pg_temp.ev('S-M-REV-16', null, null, 'context', 'Nearly 4,500 Tesla Model 3 and Model Y owners in South Korea have complained about battery management errors.'))));
...
select pg_temp.stage('M-043', 'm3_lr_awd', 'fam:m3_pack', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-30', 'thread:tmc-327730', '{"age_years":6}'::jsonb, 'supports', 'Such replacements for Model 3/Y can be around 13K-15K dollars. If you pay for it out of warranty, it has a 4yr 50K mile warranty. Vehicle High Voltage Battery: 4 years or 50,000 miles, whichever comes first.'),
      pg_temp.ev('S-M-SPEC-02', null, null, 'context', 'A Tesla Model 3 battery failure does not always mean every cell inside the pack is dead. Often, the entire battery becomes limited by one weak section, one bad module, one failed component, or one group of cells.'))));

select pg_temp.stage('M-044', 'm3_lr_awd', 'fam:m3_pack', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OFF-31', null, null, 'supports', 'Even after 200,000 miles of usage, our batteries in Model 3 and Model Y lose just 15% of their capacity on average, while batteries in Model S and Model X lose just 12% of their capacity on average.'),
      pg_temp.ev('S-M-REV-14', null, null, 'context', 'According to the 2023 Impact Report, the average battery capacity loss of the Model 3 and Model Y Long Range versions after 200,000 miles is 15%; the chart concerns only the Long Range batteries.'),
      pg_temp.ev('S-M-REV-15', null, null, 'context', 'A 2021 Model 3 Long Range All-Wheel Drive with 216,000 miles on the odometer.'))));

select pg_temp.stage('M-045', 'm3_lr_awd', 'issue:m3_pcs', 'owner_pattern', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-31', 'odi:11482849', '{"mileage_km":54700}'::jsonb, 'supports', 'A portion of the Power Conversion System has failed in my 2018 Tesla Model 3 extended range with 34,000 miles, resulting in the inability to charge the vehicle but at 32 of 48 capable amps.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11652488', '{"age_years":6}'::jsonb, 'supports', '2 of the 3 controllers have failed, resulting in limited AC charging, 16A out of 48A max. The PCS helps regulate AC and DC electricity and connects the 12V battery.'),
      pg_temp.ev('S-M-OWN-31', 'odi:11464156', '{"age_years":4}'::jsonb, 'supports', 'For a growing number of Model 3 owners, components in the HV Battery Service Panel are breaking down, yet Tesla refuses to cover the repair under its Battery and Drive Unit Warranty. Specifically, the power conversion system in that panel is failing.'))));

-- ---------- M9. Приводи ----------

select pg_temp.stage('M-046', 'm3_lr_awd', 'issue:m3_du_failure', 'owner_pattern', 'low',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-OWN-07', 'thread:too-14739', '{"mileage_km":19300}'::jsonb, 'supports', 'LR RWD built June 2018, delivered October 2018, with just over 12,000 miles. We found there to be an internal failure of the rear drive unit. We have ordered the new drive unit.'),
      pg_temp.ev('S-M-OWN-08', 'thread:tmc-349486', '{"mileage_km":226900}'::jsonb, 'supports', 'So my rear drive unit failed at 141,000 miles. It made whirring noises for about a month and then failed. The gear also machined and rotated the input shaft bearing.'),
      pg_temp.ev('S-M-OWN-06', 'thread:tmc-219672', '{"age_years":1}'::jsonb, 'supports', 'We received our Tesla 3 last Dec/Jan, and just had the rear drive unit fail for the second time. First time, Tesla said, oh, extremely rare, will not happen again.'),
      pg_temp.ev('S-M-REV-17', null, null, 'context', 'The dual-motor versions of the Model 3, Long Range and Performance, each use the same 3D3 front motor; the Long Range gets the 3D7 rear motor from the RWD variants, whereas the Performance gets the more powerful 3D6 motor.'))));

select pg_temp.stage('M-047', 'm3_lr_awd', 'fam:m3_rdu', 'specialist_practice', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-SPEC-01', null, null, 'supports', 'Drive Unit Bearing Replacement: If you hear strange sounds from the motor, it may be a sign that the drive unit bearing needs replacement. Reduction Gear Oil Flush: We advise changing the oil every 50,000 miles. Drive Unit Preventative Care: rotor seal replacement, bearing replacement, and fluid flush, backed by a 2-year or 20,000-mile warranty.'),
      pg_temp.ev('S-M-OFF-09', null, null, 'contradicts', 'Your vehicle should generally be serviced on an as-needed basis; the listed intervals are brake fluid, desiccant, cabin filter, wipers, caliper cleaning and tyre rotation.'))));
...
-- з версією RWD.
select pg_temp.stage('M-048', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(
      pg_temp.ev('S-M-REV-17', null, null, 'supports', 'The Long Range 3D7 rear motor makes 194kW/340Nm whereas the Performance 3D6 rear motor makes 220kW/440Nm. Again, each has the same 137kW/219Nm front motor. Those codes are stamped on the casing of the motor.'),
      pg_temp.ev('S-M-MKT-01', null, null, 'context', 'Drive unit part listings 1120980 and 1120990-00-J low current rear drive unit assembly.'))));

-- ---------- M10. Обслуговування ----------

select pg_temp.stage('M-049#a', 'm3_lr_awd', 'maint:m3_brake_fluid', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-09', null, null, 'supports', 'Brake fluid health check every 4 years (replace if necessary). Heavy brake usage due to towing, mountain descents, or performance driving, especially for vehicles in hot and humid environments, may necessitate more frequent brake fluid checks and replacements.'))));

select pg_temp.stage('M-049#b', 'm3_lr_awd', 'maint:m3_desiccant', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', null, 2020, 1)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-09', null, null, 'supports', 'Vehicles manufactured prior to approximately 2021: A/C desiccant bag replacement every 6 years.'))));

select pg_temp.stage('M-049#c', 'm3_lr_awd', 'maint:m3_cabin_filter', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-09', null, null, 'supports', 'Cabin air filter replacement every 2 years. Wiper blade replacements every year. The above list does not include consumable parts such as windshield wipers, brake pads, low voltage lead-acid battery.'))));

select pg_temp.stage('M-049#d', 'm3_lr_awd', 'maint:m3_brake_cleaning', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_tag('condition_tag', 'salt_climate', 1, 'tag_has')),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-09', null, null, 'supports', 'Clean and lubricate brake calipers every year or 12,500 miles (20,000 km) if in an area where roads are salted during winter. Rotate tires every 6,250 miles (10,000 km) or if tread depth difference is 2/32 in (1.5 mm) or greater.'))));

select pg_temp.stage('M-050', 'm3_lr_awd', 'gen:m3', 'official_fact', 'high',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2021, 2022, 1)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-OFF-14', null, null, 'supports', 'Some Heat Pump Model 3 vehicles were built without a hydraulic control unit wake-up wire, and in rare circumstances, braking and stability control related alerts may appear on the instrument cluster and touchscreen during the vehicle power-on process. Such faults can typically be corrected with a vehicle power cycle.'))));

-- Початок поставок повнопривідної версії відомий лише з новини: чесний блок.
select pg_temp.stage('M-051', 'm3_lr_awd', 'ver:m3_lr_awd', 'official_fact', 'medium',
  ...
  jsonb_build_object(
    ...
    'applic', jsonb_build_array(pg_temp.p_num('model_year', 2018, 2018, 1)),
    'ev', jsonb_build_array(pg_temp.ev('S-M-REV-18', null, null, 'supports', 'Tesla starts deliveries of dual-motor Model 3 in July, Musk says, 20 May 2018.'))));

-- ---------- M11. Синтези CalCar ----------
-- Опори підставляються після 130_publish.sql у 132_synthesis_tesla_model_3.sql.

select pg_temp.stage('M-052', 'm3_lr_awd', 'ver:m3_lr_awd', 'calcar_synthesis', 'high',
...
select pg_temp.stage('M-053', 'm3_lr_awd', 'ver:m3_lr_awd', 'calcar_synthesis', 'high',
